...
export class UserService {
  ...
  async create(dto: UserProto.CreateDto): Promise<UserProto.CreateAck> {
    const isNicknameTaken = await this.userRepository.isNicknameTaken(
      dto.nickname,
    );
    if (isNicknameTaken) {
  ...
  async getUserForLogin(
    dto: UserProto.GetUserForLoginDto,
  ): Promise<UserProto.GetUserForLoginAck> {
    const user = await this.userRepository.getUserByNickname(dto.nickname, [
      '+password',
    ]);
    return {
      user,
